...
void PluginLoader::on_draw_ui() {
    ...
    if (ImGui::Button(Localization::tr("Attempt Unload Plugins"))) {
        attempt_unload_plugins();
    }

    if (ImGui::Button(Localization::tr("Reload Plugins"))) {
        attempt_unload_plugins();
        reload_plugins();
    }

    if (!m_plugins.empty()) {
        ImGui::Text(Localization::tr("Loaded plugins:"));

        for (auto&& [name, _] : m_plugins) {
            ImGui::Text(name.c_str());
        }
    } else {
        ImGui::Text(Localization::tr("No plugins loaded."));
    }

    if (!m_plugin_load_errors.empty()) {
        ImGui::Spacing();
        ImGui::Text(Localization::tr("Errors:"));
        for (auto&& [name, error] : m_plugin_load_errors) {
            ImGui::Text(Localization::tr("%s - %s"), name.c_str(), error.c_str());
        }
    }

    if (!m_plugin_load_warnings.empty()) {
        ImGui::Spacing();
        ImGui::Text(Localization::tr("Warnings:"));
        for (auto&& [name, warning] : m_plugin_load_warnings) {
            ImGui::Text(Localization::tr("%s - %s"), name.c_str(), warning.c_str());
        }
    }
}
