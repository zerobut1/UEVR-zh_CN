...
void Framework::update_fonts() {
    ...
    cjk_font_config.PixelSnapH = true;

    bool added_cjk_font = false;
    for (const auto& cjk_font : {
        "C:\\Windows\\Fonts\\msyh.ttc",
        "C:\\Windows\\Fonts\\simhei.ttf",
        "C:\\Windows\\Fonts\\simsun.ttc",
    }) {
        if (fs::exists(cjk_font)) {
            fonts->AddFontFromFileTTF(cjk_font, (float)m_font_size, &cjk_font_config, fonts->GetGlyphRangesChineseFull());
            added_cjk_font = true;
            break;
        }
    }
    ...
    if (!added_cjk_font) {
        SPDLOG_WARN("[Framework] No Chinese-capable system font found; CJK UI glyphs may be missing.");
    }
...
void Framework::draw_ui() {
    ...
    Localization::set_language(FrameworkConfig::get()->get_ui_language_value());
    ...
    ImGui::Checkbox(Localization::tr("Transparency"), &m_ui_option_transparent);
    ImGui::SameLine();
    ImGui::Text(Localization::tr("(?)"));
    if (ImGui::IsItemHovered()) {
        ImGui::SetTooltip(Localization::tr("Makes the UI transparent when not focused."));
    }
    ImGui::Checkbox(Localization::tr("Input Passthrough"), &m_ui_passthrough);
    ImGui::SameLine();
    ImGui::Text(Localization::tr("(?)"));
    if (ImGui::IsItemHovered()) {
        ImGui::SetTooltip(Localization::tr("Allows mouse and keyboard inputs to register to the game while the UI is focused."));
    }
    ...
    ImGui::Text(Localization::tr("(?)"));
    if (ImGui::IsItemHovered()) {
        ImGui::SetTooltip(Localization::tr("Show additional options for greater control over various settings."));
    }

    if (m_mods_fully_initialized) {
        if (ImGui::Button(Localization::tr("Reset to Default Settings"))) {
            reset_config();
        }
    }
    ...
    ImGui::BeginGroup();
    ImGui::Text(Localization::tr("Keyboard Menu Key: Insert"));
    ImGui::Text(Localization::tr("Gamepad L3 + R3: Toggle Menu"));
    ImGui::Text(Localization::tr("Gamepad RT: Shortcuts"));
    ImGui::Text(Localization::tr("Gamepad LB/RB: Change Sidebar Page"));
    ...
    if (ImGui::BeginTable("UEVRTable", 2, ImGuiTableFlags_::ImGuiTableFlags_BordersInnerV | ImGuiTableFlags_::ImGuiTableFlags_BordersOuterV | ImGuiTableFlags_::ImGuiTableFlags_SizingFixedFit)) {
        ...
        auto dcs = [&](const char* label, int32_t page_value) -> bool {
            ...
            if (ImGui::Selectable(Localization::tr(label), m_sidebar_state.selected_entry == page_value)) {
        ...
        if (m_error.empty() && m_game_data_initialized) {
            ...
            ImGui::TextWrapped(Localization::tr("Framework is currently initializing..."));
            ImGui::TextWrapped(Localization::tr("This menu will close after initialization if you have the remember option enabled."));
            ...
            ImGui::TextWrapped(Localization::tr("Framework error: %s"), m_error.c_str());
        }
...
void Framework::draw_about() {
    ImGui::Text(Localization::tr("Author: praydog"));
    ImGui::Text(Localization::tr("Unreal Engine VR"));
    ImGui::Text(Localization::tr("https://github.com/praydog/UEVR"));
    ImGui::Text(Localization::tr("http://praydog.com"));
    ImGui::Text(Localization::tr("Branch: %s"), UEVR_BRANCH);
    ImGui::Text(Localization::tr("Commits: %i"), UEVR_TOTAL_COMMITS);
    ImGui::Text(Localization::tr("Commit hash: %s"), std::format("{:.8}", UEVR_COMMIT_HASH).c_str());
    ImGui::Text(Localization::tr("Tag: %s"), UEVR_TAG);
    ImGui::Text(Localization::tr("Commits past tag: %i"), UEVR_COMMITS_PAST_TAG);
    ImGui::Text(Localization::tr("Build date: %s"), UEVR_BUILD_DATE);
    ImGui::Text(Localization::tr("Build time: %s"), UEVR_BUILD_TIME);

    if (ImGui::CollapsingHeader(Localization::tr("Licenses"))) {
